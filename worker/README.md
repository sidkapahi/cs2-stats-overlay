...
→ 200 {
    ...
    "avatarUrl": "https://…", "elo": 3200, "eloDiff": 25, "level": 10, "region": "EU",
...
last match's ELO swing (the widget's TOTAL-mode loss/gain) — the Data API has no
per-match ELO, so it's derived best-effort from FACEIT's public web stats API and
comes back `0` when that can't be reached. A Steam account
