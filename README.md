...
OBS browser source widget that displays your CS2 Premier rating, rank badge,
stats, and recent match history. Data is fetched from the Leetify API. The
layout follows the [CS2 Stats Overlay Figma design](https://www.figma.com/design/5JQex6PZfwQGDoB4D3SrAl/CS2-Stats-Overlay).

### [Open Customizer](https://sidkapahi.github.io/cs2-stats-overlay/)
...
- Live Premier rating with a per-tier **rank badge** (Gray, Light Blue, Blue,
  Purple, Pink, Red, Gold) — or a plain rank-coloured number when the badge is
  turned off
- Rank-point change from the last Premier match (e.g. `+250`)
- Win / loss pills and stats: win rate, aim rating, K/D
- Recent match-history strip (W / L / T)
- Customizer UI to configure and generate widget URLs — including a toggle for
  the rank badge
